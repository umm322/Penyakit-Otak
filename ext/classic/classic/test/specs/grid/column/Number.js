topSuite("Ext.grid.column.Number", ['Ext.grid.Panel', 'Ext.grid.column.Date', 'Ext.form.field.Date', 'Ext.grid.plugin.CellEditing', 'Ext.grid.column.Number', 'Ext.form.field.Number'], function() {
    ...
    function expectDefaultValue(column, rowIdx, colIdx) {
        var cell = getCellInner(rowIdx, colIdx);

        expect(cell.textContent || cell.innerText).toBe('0.00');
    }
    ...
    describe("handling number column after data deletion", function() {
        beforeEach(function() {
        ...
        it("should show empty cell text on removing data from the number column with data type is string", function() {
            triggerCellMouseEvent('dblclick', 0, 4);
            expect(plugin.editing).toBe(true);
            expect(plugin.getActiveColumn()).toBe(colRef[4]);
            ...
            runs(function() {
                expectEmptyText(colRef[4], 0, 4);
                expect(plugin.editing).toBe(false);
            });
        });

        it("should show 0.00 as cell text on removing data from the number column with data type is number", function() {
            triggerCellMouseEvent('dblclick', 0, 5);
            expect(plugin.editing).toBe(true);
            expect(plugin.getActiveColumn()).toBe(colRef[5]);
            ...
            runs(function() {
                expectDefaultValue(colRef[5], 0, 5);
                expect(plugin.editing).toBe(false);
            });
        });
    });
