...
(function () {
            ...
            return function (me) {
                ...
                if (args.isEmpty) {
                    ...
                    return function () {
                        return method.apply(
                            me,
                            arguments.length ? args.concat(slice.call(arguments)) : args
                        );
                    };
                }
            };
    ...
    var Remote = (Test.Remote = {
        ...
        queueMessage: function (message, callback, notify) {
            // If we are using the headless remote runner there is no need to queue messages as they
            // are not going to a server, this is all local.
            if (Remote.headless) {
                Remote.sendMessage(message, callback);
                return;
            }
        ...
        sendMessage: function (message, callback) {
            ...
            if (Remote.headless) {
                Remote._doHeadLessSend(message);
        ...
        sendCommand: function (command, callback) {
            ...
            if (Remote.headless) {
                ...
                Remote._doSend(Remote.commandURL, command, function (success, error) {
                    if (success) {
                        Remote._setStatus(
                            "Driver command '" + type + "' executed successfully.",
                            true
                        );
                    } else {
                        Remote._setStatus(
                            "Error executing driver command '" + type + "': " + error
                        );
                    }
        ...
        _doHeadLessSend: function (messages) {
            if (!window.report) return;

            window.report(messages);
        },
        ...
        _flushQueue: function (callback) {
            ...
            if (Remote.headless) {
                Remote._doHeadLessSend(messages);
                Remote.runCallback(callback, true);
            } else {
                Remote._doSend(Remote.resultURL, messages, function (success, error) {
                    if (success) {
                        Remote._setStatus(
                            'Successfully flushed result message queue to server.',
                            true
                        );
                    } else {
                        Remote._setStatus(
                            'Error flushing result message queue to server: ' + error
                        );
                    }
        ...
        _notifyWebDriver: function (callback) {
            if (
                Remote._queueSize > Remote.queueSizeThreshold ||
        ...
        format: function (value) {
            ...
            try {
                ...
                // Assignment in conditional is intended
                    ...
                    this.append(
                        '<circular reference: ' +
                            (Test.util.isArray(value) ? 'Array' : 'Object') +
                            '>'
                    );
            ...
            for (property in obj) {
                ...
                fn.call(
                    ...
                    obj.__lookupGetter__
                        ? obj.__lookupGetter__(property) !== undefined &&
    ...
    Test.OptionsImpl.prototype.urlEncode = function (object, skipOwn) {
        ...
        for (property in object) {
            ...
            if (skipOwn && (property === 'remote-test' || property === 'headless')) {
                continue;
            }
    ...
    Test.panel.TabPanel = function (config) {
        ...
        if (document.addEventListener) {
            me.body.addEventListener('scroll', me.panelScrollListener, {
                capture: true,
                passive: true
            });
        } else {
    ...
    Test.panel.TreeGrid = function (config) {
                                    ...
                                    },
                                    {
                                        ...
                                        children: [
                                            ...
                                            },
                                            {
                                                ...
                                                html: '&#10008;',
                                                title:
                                                    'Run only failed specs in the current set\n' +
                                                ...
                                                onmousedown: function (e) {
                                                    ...
                                                    options = Test.Options.collectFailed.call(
                                                        me,
                                                        limit
                                                    );
                                                    ...
                                                    window.location.href =
                                                        '?' +
                                                        Test.Options.formLoadUrl(false, options);
                                                }
                                            },
                                            {
                                                tag: 'a',
                                            ...
                                            },
                                            {
                                                ...
                                                onmousedown: function (e) {
                                                    if (e.button === 0) {
                                                        var chunk = prompt(
                                                                'Enter chunk descriptor (e.g., "0/3")'
                                                            ),
                                                            qs;
        ...
        if (document.addEventListener) {
            ...
            body.addEventListener('scroll', me.bodyScrollListener, {
                capture: true,
                passive: true
            });
        } else {
    ...
    Test.panel.TreeGrid.prototype.checkUpdateTimer = function () {
        if (Test.STATUS_UPDATE_INTERVAL && !this.statusUpdateTimer) {
            this.statusUpdateTimer = setInterval(
                this.updateRunningSpec.bind(this),
                Test.STATUS_UPDATE_INTERVAL
            );
        }
    };
    ...
    Test.panel.TreeGrid.prototype.resizeBody = function () {
        ...
        height -=
            this.resizer.offsetHeight + this.tabPanel.el.offsetHeight + this.header.offsetHeight;
        height -= 2;
    ...
    Test.Reporter = function (config) {
        ...
        if (this.options['headless']) {
            Test.Remote.setOptions(this.options['headless']);
            Test.Remote.enabled = true;
            Test.Remote.headless = true;
        }
            ...
            if (Test.Remote.enabled) {
                ...
                for (i = 0, len = items.length; i < len; i++) {
                    ...
                    if (!item.passed()) {
                        ...
                        msg.details =
                            item.error && item.trace
                                ? item.error + ' ' + item.trace.stack
                                : item.error
                                ? item.error
                                : item.trace
                                ? item.trace
